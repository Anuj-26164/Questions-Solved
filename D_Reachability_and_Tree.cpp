...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        if (adj[i].size() == 2)
        {
            node1 = adj[i][0], node2 = i, node3 = adj[i][1];
            break;
        }
    }
    if (node1 == -1)
    ...
    newadj[node1].push_back(node2), newadj[node2].push_back(node3);
    ...
    while (!q.empty())
    {
        ...
        for (auto child : adj[node])
        {
            if (!visited[child])
            {
                ...
                if (level % 2 == 0)
                    newadj[node].push_back(child);
                else
                    newadj[child].push_back(node);
            }
        }
    }
    yes();
