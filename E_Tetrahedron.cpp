...
const int N = 10000100, M = 11, MOD = 1e9 + 7;
ll n, m;

ll numways(int idx, int d, vector<vector<int>> &dp)
{
    if (idx == n - 1 && d == 0)
        return 3;
    else if (idx == n - 1 && d == 1)
    ...
    if (dp[idx][d] != -1)
        return dp[idx][d];
    if (d == 0)
        return dp[idx][d] = (numways(idx + 1, 1, dp) + 2 * numways(idx + 1, 0, dp) + MOD) % MOD;
    else
        return dp[idx][d] = (3 * numways(idx + 1, 0, dp)) % MOD;
}
void solve()
{
    cin >> n;
    ...
    {
        long long dp0, dp1;
        long long dp0_next = 3;
        long long dp1_next = 0;
        for (long long idx = n - 3; idx >= 0; idx--)
        {
            dp0 = (dp1_next + 2LL * dp0_next) % MOD;
            dp1 = (3LL * dp0_next) % MOD;
