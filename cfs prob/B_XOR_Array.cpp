...
void zeroXorSubarrays(vector<ll> &a)
{
    int n = (int)a.size();
    vector<pair<int, int>> ans;

    unordered_map<int, vector<int>> pos; // prefXor -> all indices where it occurred
    int pref = 0;

    // pref = 0 is considered at index -1 (before array starts)
    pos[0].push_back(-1);
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        // if we've seen this prefix xor before,
        // then every previous occurrence gives a subarray with xor 0
        if (pos.count(pref))
        {
            for (int idx : pos[pref])
            {
                // subarray (idx+1 ... i) has xor = 0
