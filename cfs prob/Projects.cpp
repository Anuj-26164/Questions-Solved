...
// ll n, m, dp[M][N][N];
ll n;
ll maxreward(int idx, vector<pair<pll, ll>> &vp,vl&dp)
{
    if (idx > n)
        return 0;
    if(dp[idx]!=-1)
        return dp[idx];
    pll key = {vp[idx].ff.ss+1, LLONG_MIN};
    auto it = lower_bound(vp.begin(), vp.end(), make_pair(key, LLONG_MIN)) - vp.begin();
    ll take = vp[idx].ss + maxreward(it, vp,dp);
    ll nottake = maxreward(idx + 1, vp,dp);
...
void solve()
{
    cin >> n;
    vl dp(n + 1, -1);
    vector<pair<pll, ll>> vp(n + 1);
    f(1, n + 1) cin >> vp[i].ff.ff >> vp[i].ff.ss >> vp[i].ss;
    sort(all(vp));
    cout << maxreward(1, vp,dp) << nl;
}
int main()
{
