...
ll n, m, dp[M][N][N];
ll cnt(int idx, int num1, int num2)
{
    ...
    if (idx > m)
        return 1;
    if (dp[idx][num1][num2] != -1)
        return dp[idx][num1][num2];
    return dp[idx][num1][num2] = (cnt(idx + 1, num1, num2) + cnt(idx, num1 + 1, num2) + cnt(idx, num1, num2 - 1) - cnt(idx, num1 + 1, num2 - 1) + MOD) % MOD;
}
void solve()
{
    cin >> n >> m;
