...
template <typename T>
void operator<<(ostream &os, const set<T> &s)
{
    bool f = true;
    for (auto &x : s)
        os << (f ? "" : " ") << x, f = false;
...
const int N = 100100, M = 110, MOD = 1e9 + 7;
ll n, m, dp[M][N];
set<ll> st;

void moneysums(int idx, ll sum, vector<ll> &v)
{
    if (idx == n)
    {
        st.insert(sum);
    ...
    if (dp[idx][sum]!=-1)
        return; 
    dp[idx][sum] = 1;
    moneysums(idx + 1, sum + v[idx], v);
    moneysums(idx + 1, sum, v);
}
