...
void solve()
{
    int n, m, query;
    cin >> n >> m >> query;
    vector<vector<int>> costmatrix(n + 1, vector<int>(n + 1, INF));
    for (int i = 0; i < m; i++)
    {
        int u, v, c;
        cin >> u >> v >> c;
        costmatrix[u][v] = min(costmatrix[u][v],c);
        costmatrix[v][u] = min(costmatrix[v][u],c);
    }
    f(i, 1, n + 1) costmatrix[i][i] = 0;
    f(k, 1, n + 1)
    {
        f(i, 1, n + 1)
        {
    ...
    while (query--)
    {
        int x, y;
        cin >> x >> y;
        cout << (costmatrix[x][y]==4e18?-1:costmatrix[x][y]) << nl;
    }
}
int32_t main()
{
