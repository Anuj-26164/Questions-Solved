// Qs: Dynamic_Range_Minimum_Queries
// Time: 20:51:15
...
long long query(int idx, int l, int r, int lq, int rq)
{
    // no overlap
    if (rq < l || lq > r)
        return (long long)4e18; // INF
    ...
    int mid = (l + r) / 2;
    long long left = query(2 * idx + 1, l, mid, lq, rq);
    long long right = query(2 * idx + 2, mid + 1, r, lq, rq);
...
void solve()
{
    ...
    while (m--)
    {
        ...
        {
            l--, r--;
            cout << query(0, 0, n - 1, (int)l, (int)r) << '\n';
        }
        else
        {
            // ch == 1: point update, set a[l] = r
            l--;
            update(0, 0, n - 1, (int)l, r);
        }
    }
}
