// Qs: B_Distributed_Debt_Settlement
// Time: 14:20:22
...
void solve()
{
    ...
    vector<pair<int, long long>> d, c;

    for (int i = 1; i <= n; i++)
    {
        if (balance[i] < 0)
            d.push_back({i, -balance[i]});
        else if (balance[i] > 0)
            c.push_back({i, balance[i]});
    }

    vector<tuple<int, int, long long>> ans;

    int i = 0, j = 0;
    while (i < (int)d.size() && j < (int)c.size())
    {
        long long x = min(d[i].second, c[j].second);

        ans.emplace_back(d[i].first, c[j].first, x);
